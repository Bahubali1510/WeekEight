package src;/*Write a java program to input student Name, roll No, and three subjects Math, Science and English
marks (marks is between 0 to 100 and if it is out of range print error message “InvalidInput, Marks
should between 0 to 100”) and find out total, percentage and result.
If he is pass or fail on basis of percentage (pass>=35) and also give them grade if %> = 80 A+,
%> = 60 A, %> = 50 B, %> = 35 C And print Mark Sheet in following format*/

import java.util.Scanner;

public class Program5 {
    String StudentName;
    int RollNumber;
    double marks;
    double percentage;
   // double  subjects= Math, Science, English;

    public static void main(String[] args) {
        Scanner sc = new Scanner(System.in);

        System.out.println("Name");
        String StudentName = sc.next();

        System.out.println("Roll No ");
        int RollNumber = sc.nextInt();

        System.out.println("Maths");
        double mathMarks = sc.nextDouble();

        System.out.println("Science");
        double scienceMarks = sc.nextDouble();

        System.out.println("English");
        double englishMarks = sc.nextDouble();

        Double totalMarks = mathMarks+scienceMarks+englishMarks;
        double percentage= (totalMarks/300)*100;

        System.out.println("|-------------------|");
        System.out.println("|     Mark Sheet    |");
        System.out.println("|-------------------|");
        System.out.println("| Name            : " + StudentName);
        System.out.println("| Roll No         : " + RollNumber);
        System.out.println("|-------------------|");
        System.out.println("| Subjects : Marks |");
        System.out.println("|-------------------|");
        System.out.println("| Math    : " + mathMarks);
        System.out.println("| Science : " + scienceMarks);
        System.out.println("| English : " + englishMarks);
        System.out.println("|-------------------|");
        System.out.println("| Total    : " + totalMarks);
        System.out.println("|-------------------|");
        System.out.println("| Percentage: " + percentage);
        System.out.println("| Result    : " + (percentage >= 35 ? "Pass" : "Fail"));

        if (percentage<35){
            System.out.println("F");
        } else if (percentage >=35) {
            System.out.println("C");
        } else if (percentage>=50) {
            System.out.println("B");            
        } else if (percentage>=60) {
            System.out.println("A");
        } else if (percentage>=80) {
            System.out.println("A+");
        }
        else {
            System.out.println("Invalid Input");
        }


    }
}
